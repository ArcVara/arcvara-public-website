        ...
        <div className="prose-legal">
          {sections.map((section, i) => (
            <LegalSection key={i} title={section.title}>
              ...
              {i === 8 && (
                <p className="text-vara-teal">
                  <Link to={`/${lang}/data-rights`} className="hover:underline">
                    → {t('footer.dataRights')}
                  </Link>
                </p>
              )}
...
export function LegalFooter({ lang }) {
  ...
  return (
    <div className="mt-16 pt-8 border-t border-arc-blue/10 flex flex-wrap gap-6 text-xs text-white/30">
      <Link to={`/${lang}/privacy-policy`} className="hover:text-white/60 transition-colors">{t('footer.privacy')}</Link>
      <Link to={`/${lang}/terms`} className="hover:text-white/60 transition-colors">{t('footer.terms')}</Link>
      <Link to={`/${lang}/data-rights`} className="hover:text-white/60 transition-colors">{t('footer.dataRights')}</Link>
      <span className="ms-auto">© {new Date().getFullYear()} Arcvara</span>
    </div>
  )
}
